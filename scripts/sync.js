...
if (!fs.existsSync(path.resolve(__dirname, '../../antd-components'))) {
  console.error('未找到迁移目标项目：antd-components，请检查后重试');
  return;
}
...
compPaths.forEach(pathItem => {
  const targetPath = pathItem.replace('/cantd-demo/src/components', '/cantd/src');
  try {
